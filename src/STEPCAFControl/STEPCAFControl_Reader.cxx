                     ...
                     const Handle(StepVisual_StyledItem)& theStyle)
{
  ...
  StepVisual_StyledItemTarget anItem = theStyle->ItemAP242();
  if (!anItem.IsNull()) {
                                    ...
                                    Standard_Boolean theIsRoot)
{
  ...
  for(; aSubs.More(); aSubs.Next())
  {
    ...
    if(!anOverRidingStyle.IsNull())
    {
      ...
      Handle(Standard_Transient) anItem = anOverRidingStyle->ItemAP242().Value();
  ...
  Bnd_Box& theBox)
{
  ...
  for (; i < anAnnotations.Length(); i++)
  {
    ...
    if (!anAO.IsNull())
    {
      const Handle(StepRepr_RepresentationItem)& aCurveItem = anAO->Item();
    ...
    {
      const Handle(StepRepr_RepresentationItem)& aTessItem = anItem->Item();
      if (aTessItem.IsNull())
        continue;
